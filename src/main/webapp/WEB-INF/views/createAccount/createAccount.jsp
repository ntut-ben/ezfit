	...
	<table id="login" valign="top">
		<tr>
			<td id="loginLeft">加入會員
				...
				<form action="${pageContext.request.contextPath}/api/createAccount/memberServlet" method="post"
					id="theForm">
					<input type="text" class="accountInfo" placeholder="姓名"
						id="memberName" name="memberName"
						value="${requestScope.name}${param.memberName}"> <input
						type="email" class="accountInfo" placeholder="Email"
						id="memberEmail" name="memberEmail"
						value="${requestScope.email}${param.memberEmail}">
					<div class="error">${MsgMap.errorEmail}</div>
					<input type="password" class="accountInfo"
						placeholder="密碼 (8~12位元, 需包含英文大小寫及數字)" id="memberPassword"
						name="memberPassword"
						value="${requestScope.password}${param.memberPassword}"> <input
						type="password" class="accountInfo" placeholder="確認密碼"
						id="memberPasswordCheck" name="memberPasswordCheck"> <input
				...
				<div class="fb-login-button" data-width="380" data-size="medium"
					data-button-type="login_with" data-auto-logout-link="false"
					data-use-continue-as="false">Facebook登入</div>
