...
function getPlaneProduct() {
  $.ajax({
    type: "GET",
    cache: false,
    url: `http://localhost:8080/ezfit/api/planeProductRetrieve`,
    dataType: "json",
...
function main(breakfastArray, lunchArray, dinnerArray) {
  $.ajax({
    type: "POST",
    cache: false,
    url: `http://localhost:8080/ezfit/api/mealBox/${plane}`,
    data: $("#planeForm").serialize(),
    dataType: "json",
    success: function(data) {
      $("#lunchRow")
        .children(".col-2-c")
    ...
    complete: function(XMLHttpRequest, textStatus) {
      ...
      $(".changeMeal").click(function(e) {
        ...
        href = `modifyMeals?plane=${plane}&modifyMeal=true&meal=${meal}&meals=${workoutPlaneMeals}&shipDate=${shipDate}&days=${days}`;
        whichDay = "";
        for (let index = 0; index < days; index++) {
          switch (index) {
            case 0:
...
function cart(cartBtn) {
  ...
  $.ajax({
    type: "POST",
    url: "http://localhost:8080/ezfit/api/shopCart/planeAdd",
