...
$(document)
		.ready(
				function() {
					...
					data = [ {
						id : '會員名稱',
						detail : '留言內容。作者可直接回覆留言 or not ？',
						time : '2天前 （或是直接顯示日期＋時間？）'
					}, ];
					...
					$.ajax({
						...
						success : function(data) {
							...
							if (login.memberImage !== undefined) {
								head2.setAttribute('src',
										'/ezfit/image/memberHead/'
												+ login.memberImage + '/jpg');
							} else {
								// 預設有點醜，考慮要不要換
								head2.setAttribute('src',
										'index/img/nav/logo-g.svg');
							}
					...
					// 未收藏
					let unsave = '<img src="img/recipe_page/icon-heart-solid.svg" class="pb-1 pr-1" alt="" >'
							+ ' 收 藏 ';
					// 已收藏
					let saved = '<img src="img/recipe_page/icon-heart-solid.svg" class="pb-1 pr-1" alt="" >'
							+ ' 已 收 藏 ';
															...
															$
																	.ajax({
																		...
																		success : function(
																			...
																			let recipePic = document
																					.getElementById('recipePic');

																			// recipePic.setAttribute('src',
																			// '/ezfit/image/recipe/'
																			// +
																			// list[0].fileName);
																			// let
																			...
																			// =
																			// '<img
																			// src="img/recipe_page/icon-servings.svg"
																			// alt=""
																			// >份量：'
																			// +
																			// list[0].servings
																			...
																			// spendTime
																			// =
																			// document.getElementById('spendTime');
																			// spendTime.innerHTML
																			// =
																			// '<img
																			// src="img/recipe_page/icon-cookingtime.svg"
																			// alt=""
																			// id="spendTime">時間：'
																			// +
																			// list[0].spendTime
															...
															$
																	.ajax({
																		...
																		success : function(
																			...
																			// recipePic
																			// =
																			// document.getElementById('recipePic');
																			// if
																			// ((list[0].fileName
																			// !==
																			// null
																			...
																			// {
																			// recipePic.setAttribute('src',
																			// 'img/publish_recipe/add_photo_small.svg')
																			// }
																			// let
																			// introduction
																			// =
					...
					$
							.ajax({
								...
								success : function(data) {
									...
									let recipePic = document
											.getElementById('recipePic');
									console.log('recipePic.fileName = '
											+ list[0].fileName);
									let head = document
											.getElementById('memberHeadPic');
									console.log('memberPic='
											+ list[0].member.memberImage);
									...
									servings.innerHTML = '<img src="img/recipe_page/icon-servings.svg" alt="" >份量：'
											+ list[0].servings + '人份';
									let spendTime = document
											.getElementById('spendTime');
									spendTime.innerHTML = '<img src="img/recipe_page/icon-cookingtime.svg" alt="" id="spendTime">時間：'
											+ list[0].spendTime + '分鐘';
									...
									for (let i = 0; i < materalList.length; ++i) {
										...
										ingredient.setAttribute('class',
												'ingredient');
										let materalName = document
												.createElement('div');
										materalName.setAttribute('class',
												'ingredient-name');
										// materalName.innerHTML =
										// materalList[i].materalName;
										let linkMateral = document
												.createElement('a');
										linkMateral.setAttribute('href', '#');
										linkMateral.innerHTML = materalList[i].materalName;

										let materalUnit = document
												.createElement('div');
										materalUnit.setAttribute('class',
												'ingredient-unit');
										// materalUnit.innerHTML =
										// materalList[i].materalName;
										let linkUnit = document
												.createElement('a');
										...
										// 將食材名裝進array中
										materal
												.push(materalList[i].materalName);
									}
									...
									for (let i = 0; i < methodList.length; ++i) {
										// parentNode
										let methodSec = document
												.getElementById('methodSec');

										// 動態新增
										let instructionStep = document
												.createElement('div');
										instructionStep.setAttribute('class',
												'instruction-step');

										// 圖片區
										let stepImg = document
												.createElement('div');
										stepImg.setAttribute('class',
												"step-img");
										let methodPic = document
												.createElement('img');
										if ((methodList[i].fileName !== null && (methodList[i].fileName !== undefined))) {
											let str = (methodList[i].fileName)
													.split('.');
											methodPic
													.setAttribute(
															'src',
															'/ezfit/image/method/'
																	+ methodList[i].fileName
																	+ '/'
																	+ str[str.length - 1]);
										} else {
											methodPic
										...
										let stepNumber = document
												.createElement('div');
										stepNumber.setAttribute('class',
												"step-number");
										stepNumber.innerHTML = i + 1;
										let stepInstruction = document
												.createElement('div');
										stepInstruction.setAttribute('class',
												"step-instruction");
									...
									// 作者相關
									let memberLink = document
											.getElementById('memberLink');
									...
									// document.getElementById('memberLink1').href
									// =
									// `/WEB-INF/views/my_page.jsp?ownerId=${list[0].member.pkey}&page=1&year=2019`;
									// document.getElementById('memberLink2').href
									// =
									// `/WEB-INF/views/my_page.jsp?ownerId=${list[0].member.pkey}&page=1&year=2019`;
					...
					$.ajax({
						...
						success : function(data) {
							...
							// 更新筆數
							let chatTotal = document
									.getElementById('chatTotal');
											...
											$
													.ajax({
														...
														success : function(data) {
															...
															// // 更新筆數
															// let chatTotal =
															// document.getElementById('chatTotal');
															// chatTotal.innerHTML
															// = '共 ' +
															// chatLength + '
															// 則';
															...
															// method: "GET",
															// url:
															// 'http://localhost:8080/ezfit/recipe/editRecipe',
															// data: {
															// recipeId:
															// recipeId,
															// },
...
function createChatSec(data) {
	...
	for (let i = 0; i < data.length; ++i) {
		...
		if (data[i].member.memberImage !== undefined) {
			...
			memberPic.setAttribute('src', '/ezfit/image/memberHead/'
					+ data[i].member.memberImage + '/jpg');
		} else {
			// 預設有點醜，考慮要不要換
