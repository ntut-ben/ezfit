...
function displayContacts(crContact) {
	...
	if (crContact) {
		crContact.uids.forEach(function(uid) {
			var msgItem = $("<li/>").addClass("list-group-item").text(uid)
...
function appendCrMessage(messagelist, msg) {
	var msgItem = $("<li/>").addClass("list-group-item").appendTo(messagelist);
...
function displayAlert(crAlert) {
	if (crAlert) {
		...
		alertdiv.removeClass("alert-info alert-warning d-none").addClass(
				"alert-" + crAlert.type).addClass("d-block");
	}
}
