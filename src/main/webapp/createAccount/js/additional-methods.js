	...
	if (typeof define === "function" && define.amd) {
		...
		(function($) {
			...
			$.validator.addMethod("accept", function(value, element, param) {
				...
				// Split mime on commas in case we have multiple types we can
				// accept
				var typeParam = typeof param === "string" ? param.replace(
						/\s/g, "") : "image/*", optionalValue = this
						.optional(element), i, file, regex;
				...
				if ($(element).attr("type") === "file") {
					...
					// Check if the element has a FileList before checking each
					// file
					if (element.files && element.files.length) {
						...
						for (i = 0; i < element.files.length; i++) {
							...
							// Grab the mimetype from the loaded file, verify it
							// matches
			...
			$.validator.addMethod("upperandlowercase",
			 ...
			 * optional)
			 * 
			 * Validation is case-insensitive. Please make sure to normalize
			 * input yourself.
			 * 
			 ...
			 * must be a letter ('O' is not allowed) or digit ('0' for test
			 * (therefore not allowed), '1' denoting passive participant, '2'
			 * typically reverse-billing) - Last 3 characters - branch code,
			 * optional (shall not start with 'X' except in case of 'XXX' for
			 ...
			 * Where:
			 * 
			 * T: 1 character. Kind of Organization Letter:
			 * [ABCDEFGHJKLMNPQRSUVW] P: 2 characters. Province. N: 5
			 * characters. Secuencial Number within the province. C: 1
			 * character. Control Digit: [0-9A-J].
			 *  [ T ]: Kind of Organizations. Possible values:
			 * 
			 * A. Corporations B. LLCs C. General partnerships D. Companies
			 * limited partnerships E. Communities of goods F. Cooperative
			 * Societies G. Associations H. Communities of homeowners in
			 * horizontal property regime J. Civil Societies K. Old format L.
			 * Old format M. Old format N. Nonresident entities P. Local
			 * authorities Q. Autonomous bodies, state or not, and the like, and
			 * congregations and religious institutions R. Congregations and
			 * religious institutions (since 2008 ORDER EHA/451/2008) S. Organs
			 * of State Administration and regions V. Agrarian Transformation W.
			 * Permanent establishments of non-resident in Spain
			 *  [ C ]: Control Digit. It can be a number or a letter depending
			 * on T value: [ T ] --> [ C ] ------ ---------- A Number B Number E
			 * Number H Number K Letter P Letter Q Letter S Letter
			 * 
			 */
			$.validator
					.addMethod(
							...
							function(value, element) {
								...
								number = value.substring(1, 8), // [ P ][ P ][ N
																// ][ N ][ N ][
																// N ][ N ]
								...
								for (i = 0; i < number.length; i++) {
									...
									if (isOdd(i)) {
										...
										// If the multiplication is bigger than
										// 10 we need to adjust
										odd_sum += n < 10 ? n : n - 9;
								...
								control_digit = (10 - (all_sum).toString()
										.substr(-1)).toString();
								control_digit = parseInt(control_digit, 10) > 9 ? "0"
										: control_digit;
								control_letter = "JABCDEFGHI".substr(
										control_digit, 1).toString();
			...
			/*
			 * Brazillian CNH number (Carteira Nacional de Habilitacao) is the
			 * License Driver number. CNH numbers have 11 digits in total: 9
			 * numbers followed by 2 check numbers that are being used for
			 * validation.
			 */
			...
			/*
			 * Brazillian value number (Cadastrado de Pessoas Juridica). value
			 * numbers have 14 digits in total: 12 numbers followed by 2 check
			 * numbers that are being used for validation.
			 */
			...
			$.validator.addMethod("creditcardtypes", function(value, element,
				...
				if (validTypes
						& 0x0001
						&& (/^(5[12345])/.test(value) || /^(2[234567])/
								.test(value))) { // Mastercard
					...
					.addMethod(
							...
							function(value, element, param) {
								...
								if (this.settings.onfocusout
										&& target
												.not(".validate-greaterThan-blur").length) {
									target
											.addClass(
													"validate-greaterThan-blur")
											.on("blur.validate-greaterThan",
													function() {
														$(element).valid();
													});
								}
			...
			$.validator
					.addMethod(
							...
							function(value, element, param) {
								...
								if (this.settings.onfocusout
										&& target
												.not(".validate-greaterThanEqual-blur").length) {
									target
											.addClass(
													"validate-greaterThanEqual-blur")
											.on(
			...
			$.validator
					.addMethod(
							...
							function(value, element) {
								...
								// two check digits,
								// Basic Bank Account Number (BBAN) - up to 30
								// chars
			...
			$.validator.addMethod("lessThan", function(value, element, param) {
				...
				if (this.settings.onfocusout
					...
					target.addClass("validate-lessThan-blur").on(
							"blur.validate-lessThan", function() {
								$(element).valid();
							});
				}
			...
			$.validator.addMethod("lessThanEqual", function(value, element,
				...
				if (this.settings.onfocusout
					...
					target.addClass("validate-lessThanEqual-blur").on(
							"blur.validate-lessThanEqual", function() {
								$(element).valid();
							});
				}
			...
			$.validator
					.addMethod(
							...
							$.validator
									.format("Total size of all files must not exceed {0} bytes."));
			...
			$.validator.addMethod("mobileRU", function(phone_number, element) {
				...
				return this.optional(element) || ruPhone_number.length > 9
			...
			/*
			 * The NIE (Número de Identificación de Extranjero) is a Spanish tax
			 * identification number assigned by the Spanish authorities to any
			 * foreigner.
			 * 
			 * The NIE is the equivalent of a Spaniards Número de Identificación
			 * Fiscal (NIF) which serves as a fiscal identification number. The
			 * CIF number (Certificado de Identificación Fiscal) is equivalent
			 * to the NIF, but applies to companies rather than individuals. The
			 * NIE consists of an 'X' or 'Y' followed by 7 or 8 digits then
			 * another letter.
			 */
			$.validator
					.addMethod(
							...
							function(value, element) {
								...
								value = value.replace(/^[X]/, "0").replace(
										/^[Y]/, "1").replace(/^[Z]/, "2");

								number = value.length === 9 ? value
										.substr(0, 8) : value.substr(0, 9);

								return validChars
										.charAt(parseInt(number, 10) % 23) === letter;
			...
			/*
			 * Numer identyfikacji podatkowej ( NIP ) is the way tax
			 * identification used in Poland for companies
			 */
			 ...
			 * 
			 * @Description Brazillian PIS or NIS number (Número de
			 *              Identificação Social Pis ou Pasep) is the equivalent
			 *              of a Brazilian tax registration number NIS of PIS
			 *              numbers have 11 digits in total: 10 numbers followed
			 *              by 1 check numbers that are being used for
			 *              validation.
			 * @copyright (c) 21/08/2018 13:14, Cleiton da Silva Mendonça
			 * @author Cleiton da Silva Mendonça <cleiton.mendonca@gmail.com>
			 * @link http://gitlab.com/csmendonca Gitlab of Cleiton da Silva
			 *       Mendonça
			 * @link http://github.com/csmendonca Github of Cleiton da Silva
			 *       Mendonça
			 */
			...
			$.validator
					.addMethod("notEqualTo", function(value, element, param) {
						return this.optional(element)
								|| !$.validator.methods.equalTo.call(this,
										value, element, param);
					},
			 ...
			 * 
			 * Mobile phone numbers starts with following digits: 45, 50, 51,
			 * 53, 57, 60, 66, 69, 72, 73, 78, 79, 88.
			 * 
			 * Fixed-line numbers starts with area codes: 12, 13, 14, 15, 16,
			 * 17, 18, 22, 23, 24, 25, 29, 32, 33, 34, 41, 42, 43, 44, 46, 48,
			 * 52, 54, 55, 56, 58, 59, 61, 62, 63, 65, 67, 68, 71, 74, 75, 76,
			 * 77, 81, 82, 83, 84, 85, 86, 87, 89, 91, 94, 95.
			 * 
			 * Ministry of National Defence numbers and VoIP numbers starts with
			 * 26 and 39.
			 * 
			 * Excludes intelligent networks (premium rate, shared cost, free
			 * phone numbers).
			 * 
			 * Poland National Numbering Plan
					...
					.addMethod(
							...
							function(phone_number, element) {
								...
								return this.optional(element)
										|| phone_number.length > 9
										&& phone_number
												.match(/^(\+?1-?)?(\([2-9]([02-9]\d|1[02-9])\)|[2-9]([02-9]\d|1[02-9]))-?[2-9]\d{2}-?\d{4}$/);
							}, "Please specify a valid phone number");
